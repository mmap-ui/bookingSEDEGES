<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    <!-- Header de navegacion de la aplicacion -->
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @can('aprobar reservas')
                        <x-nav-link href="{{ route('solicitudes') }}" :active="request()->routeIs('solicitudes')">
                            {{ __('Solicitudes') }}
                            @php
                                $pendingCount = App\Models\Reservation::query()->pending()->count();
                            @endphp
                            @if ($pendingCount > 0)
                                <span
                                    class="ms-1.5 inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">
                                    ({{ $pendingCount }})
                                </span>
                            @endif
                        </x-nav-link>
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('aprobar reservas')
                <x-responsive-nav-link href="{{ route('solicitudes') }}" :active="request()->routeIs('solicitudes')">
                    {{ __('Solicitudes') }}
                    @php
                        $pendingCountResponsive = App\Models\Reservation::query()->pending()->count();
                    @endphp
                    @if ($pendingCountResponsive > 0)
                        <span
                            class="ms-1.5 inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">
                            ({{ $pendingCountResponsive }})
                        </span>
                    @endif
                </x-responsive-nav-link>
            @endcan
